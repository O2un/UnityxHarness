...
- Manager(순수 C#) → Module(순수 C#, Unity API 비의존, `new` 가능) → Service(외부 접점, 인터페이스 추상화).
  - "Unity API 비의존"의 금지 대상은 `MonoBehaviour` 상속·`Transform`/`GameObject`/컴포넌트 조작·씬 탐색이다. `Mathf`·`Random` 같은 순수 정적 유틸, `AnimationCurve` 같은 데이터 타입을 값으로 받아 쓰는 것은 위반이 아니다(리뷰에서 지적하지 말 것).
...
- **등록 방식**: 여러 소비처가 타입으로 받거나 씬 autoInject 메서드 주입이면 `RegisterInstance`. 소비처가 하나뿐이고 재사용 가능성이 없으면 그 등록의 `WithParameter(...)` 파라미터 주입을 쓴다. 아무도 주입받지 않는 `RegisterInstance`(죽은 등록)는 제거하고, `WithParameter`로 값을 다 넘기면서 원본 SO를 `RegisterInstance`하는 중복은 지적한다.
