...
이 프로젝트는 큰 Unity 개발 작업을 위한 Agent Team 하네스와 MCP for Unity 검증 흐름을 제공합니다.

## 자연어 라우팅
요청을 아래 3단계로 먼저 분류합니다. **애매하면 더 가벼운 단계에서 시작**하고, 조사 중 범위가 커진 것이 확인될 때만 상위 단계로 전환합니다.

1. **직접 처리**
   - 질문·설명·코드 리뷰·문서·설정·하네스 수정
   - 한 줄 수정뿐 아니라, 기존 구조 안에서 끝나는 단일 파일의 국소 수정
   - Agent Team, 설계 문서, `artifacts/` 파이프라인을 사용하지 않습니다.
2. **경량 개발**
   - 기존 기능의 작은 버그 수정·리팩토링·부분 기능 추가
   - 대체로 1~2개 코드 파일에 한정되고 새 시스템·공개 인터페이스·씬/프리팹 변경이 없는 작업
   - 현재 대화에서 바로 구현하고 영향 범위에 맞는 정적 검사·컴파일·관련 테스트만 수행합니다. 기본 4인 팀과 전체 설계/리뷰 산출물을 만들지 않습니다.
3. **전체 오케스트레이션**
   - 새 게임 시스템 구축, 여러 시스템/레이어를 가로지르는 변경, 다수 파일 신설, 씬·프리팹·에셋 작업, 설계 결정과 단계별 승인이 필요한 작업
   - 이 경우에만 개별 Agent를 부르기 전에 **`unity-dev-orchestrator` Skill을 사용**합니다.

"버그 고쳐줘", "리팩토링해줘", "기능 추가해줘" 같은 표현만으로 전체 오케스트레이션을 선택하지 않습니다. 실제 변경 범위와 위험도를 기준으로 판단합니다.

## 흐름 요약
전체 오케스트레이션에서만 설계(unity-architect) → **[승인: 배치 위치 공통/프로젝트]** → 구현(gameplay-engineer) → 씬·검증(unity-ai-operator, **[승인: 씬·에셋]**) → 리뷰(code-reviewer) 흐름을 사용합니다. 필요한 역할만 선택하며 항상 4명을 구성하지 않습니다. 검증은 변경 위험에 맞게 수행하고, 전체 오케스트레이션의 4단계 게이트(①컴파일 ②Play 콘솔에러 ③기능테스트 ④사용자 확인)는 ①~③을 Stop hook이 수행합니다.
...
- **새 시스템의 코드 배치 위치는 오픈 퀘스천**입니다. 전체 오케스트레이션에서 새 시스템을 만들 때 `00_CommonFramework`(공통)로 둘지 프로젝트 전용 영역에 둘지 구현 직전 사용자에게 확인합니다. 기존 파일의 국소 수정에는 이 승인 게이트를 적용하지 않습니다.
