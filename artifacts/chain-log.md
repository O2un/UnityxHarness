...
---
## [archived] 2026-07-15 — Part 1 improvement-log (축약)
...
## [축약 archive] 2026-07-16 · ProjectB 2D Player 벽 마찰·페이싱 수정

- Unitychan BoxCollider2D에 friction/bounciness 0인 `PlayerNoFriction.physicsMaterial2D`를 연결해 벽 마찰 낙하 정지를 방지했다.
- `PlayerView.LateUpdate`가 실제 Rigidbody2D 수평 속도로 SpriteRenderer 방향을 갱신하고 정지 시 마지막 방향을 유지하도록 했다.
- 정적 배선과 리뷰는 통과했으며 Unity MCP 미연결로 컴파일·Play·기능 자동검증은 수동 확인 상태였다.

## [축약 archive] 2026-07-16 · ActorManager 중앙 프레임 루프

- `IActorTickable`/`IActorFixedTickable` 기능 계약 추가, ProjectA/B `ActorManager`가 VContainer `ITickable`/`IFixedTickable`로 활성 Actor만 중앙 실행.
- Context의 Actor 직접 Update/FixedUpdate 호출 제거, NPC SkillModule도 NpcActor 소유로 이동.
- 구조 검증·리뷰 통과(blocker/major/minor 0), MCP 미연결로 Gate 1~3 수동 확인 대기였음.
